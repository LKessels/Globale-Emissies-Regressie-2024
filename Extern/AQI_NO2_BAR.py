...
PlaceComplete_DF['no2 aqi category'] = pd.Categorical(
    PlaceComplete_DF['no2 aqi category'], 
    categories=category_order, 
    ordered=True
)

grouped_data = PlaceComplete_DF.groupby(['region', 'no2 aqi category'], observed=False).size().unstack(fill_value=0)
...
plt.title('NO2 AQI Categorie per Regio')
plt.xlabel('Regio')
plt.ylabel('Hoeveelheid Steden')
plt.xticks(rotation=45)
plt.legend(title='NO2 AQI Categorie', bbox_to_anchor=(1.05, 1), loc='upper left')
